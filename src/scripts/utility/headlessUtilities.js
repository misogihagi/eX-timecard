...
function fetchResultHTML(resource,item){
        ...
        const method = "POST";
        function toSJIS(s){
          return Encoding.convert(s.split('').map(v => v.charCodeAt()),'SJIS')
        }
      ...
      function parseTodayData(doc){
        return constants.todayScreenDB.reduce((acc,cur)=>{
          acc[cur.key]=doc.querySelector(`${cur.nodeName}[name="${cur.key}"]`).value
          return acc
        },{})
      }
      async function moveTodayScreen(data){
          const {nextScreen,resultHTML}=await postLoginScreen(data)
          const afterLoginDocument = new DOMParser().parseFromString(resultHTML, "text/html");
          const nextToken=afterLoginDocument.querySelector('input[name="org.apache.struts.taglib.html.TOKEN"]').value
      ...
      async function postTodayData({token,data}){
        ...
        async function  getThisHalfMonth(){
          ...
          const afterLoginHTML = new DOMParser().parseFromString(afterLoginHTMLStr, "text/html");
          const token=afterLoginHTML.querySelector('input[name="org.apache.struts.taglib.html.TOKEN"]').value
          const dataHTMLStr=await getThisHalfMonthHTML(token)
          const dataHTML = new DOMParser().parseFromString(dataHTMLStr, "text/html");
          function indexof(e){return etimecardSchema[e].index}
          return Array.from(dataHTML.querySelectorAll('table.inputtable_02.cmWidthP100')[2]
            .querySelectorAll('tr'))
            .slice(1)
            .map(tr=>
...
function formatDate(date){return (""+date.getHours()).padStart(2,'0')+(""+date.getMinutes()).padStart(2, '0')}
